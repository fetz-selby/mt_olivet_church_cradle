...
public class AddSpecialOfferingPopup extends Composite {

	private int stageIndex = 0;
	AddSpecialOfferingPopupEventHandler handler;
	private IsWizardable<SpecialOfferingModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AddSpecialOfferingPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AddSpecialOfferingPopup> {
	}
	...
	public AddSpecialOfferingPopup() {
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.ADD_SPECIAL_OFFERING_TITLE);
		wizard = new StageOne();
	...
	private void initWizardableEvent(final IsWizardable<SpecialOfferingModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<SpecialOfferingModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, SpecialOfferingModel model) {
	...
	private void showAddSuccess(){
		...
		titleErrorMessage.setInnerText("Special Offering Added Successfully");
	}
	...
	private void doSaveOffering(SpecialOfferingModel model){
		disableButton(true);
		GlobalResources.getInstance().getAddRPC().addSpecialOffering(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	public void setAddSpecialOfferingPopupEventHandler(AddSpecialOfferingPopupEventHandler handler){
