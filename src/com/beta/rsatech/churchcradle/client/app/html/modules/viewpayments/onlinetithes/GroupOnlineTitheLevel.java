...
public class GroupOnlineTitheLevel extends Composite {

	private int yearFilter = new Date().getYear() < 1900?new Date().getYear()+1900:new Date().getYear();
	private GroupOnlineTitheLevelEventHandler handler;
	...
	interface GroupBibleReadLevelUiBinder extends
			UiBinder<Widget, GroupOnlineTitheLevel> {
	}
	
	public interface GroupOnlineTitheLevelEventHandler{
		void onMonthClicked(ArrayList<OnlineTithePaymentModel> modelList);
	}
	...
	private void doListing(String month, int year){
		GlobalResources.getInstance().getPaymentRPC().getOnlineTithePayments(GlobalResources.getInstance().getModel().getChurchId(), month+","+year, new AsyncCallback<ArrayList<OnlineTithePaymentModel>>() {
			
			@Override
			public void onSuccess(ArrayList<OnlineTithePaymentModel> result) {
				if(result != null){
					if(handler != null){
	...
	public void setGroupOnlineTitheLevelEventHandler(GroupOnlineTitheLevelEventHandler handler){
