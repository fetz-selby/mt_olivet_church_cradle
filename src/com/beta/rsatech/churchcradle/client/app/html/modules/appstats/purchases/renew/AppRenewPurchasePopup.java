...
public class AppRenewPurchasePopup extends Composite {

	private int stageIndex = 0;
	AppRenewPurchasePopupEventHandler handler;
	private IsWizardable<AppRenewModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AppRenewPurchasePopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AppRenewPurchasePopup> {
	}
	...
	@UiField DivElement loadingDiv;

	public AppRenewPurchasePopup() {
		initWidget(uiBinder.createAndBindUi(this));
		showLoading(true);
		grabCurrentExchangeRate();
	}
	...
	private void initComponent(double dollarRate){
		titleSpan.setInnerText(AppConstants.APP_RENEW_TITLE);
		wizard = new StageOne(dollarRate);
	...
	private void grabCurrentExchangeRate(){
		GlobalResources.getInstance().getListRPC().getDollarRate(new AsyncCallback<Double>() {
	...
	private void initWizardableEvent(final IsWizardable<AppRenewModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<AppRenewModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, AppRenewModel model) {
	...
	private void doAppRenewPurchase(AppRenewModel model){
		disableButton(true);
		GlobalResources.getInstance().getPaymentRPC().renewApp(model, Utils.getLocationPath("app.html"),new AsyncCallback<String>() {
	...
	public void setAppRenewPurchasePopupEventHandler(AppRenewPurchasePopupEventHandler handler){
