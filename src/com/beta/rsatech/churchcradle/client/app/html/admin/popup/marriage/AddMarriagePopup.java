...
public class AddMarriagePopup extends Composite {

	private int stageIndex = 0;
	//private MemberModel model;
	AddMarriagePopupEventHandler handler;
	private IsWizardable<MarriageModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AddMarriagePopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AddMarriagePopup> {
	}
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.ADD_MARRIAGE_TITLE);
		wizard = new StageOne();
	...
	private void initWizardableEvent(final IsWizardable<MarriageModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<MarriageModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, MarriageModel model) {
	...
	private void showAddSuccess(){
		...
		titleErrorMessage.setInnerText("Marriage Added Successfully");
	}
	...
	private void doSaveMarriage(MarriageModel model){
		disableButton(true);
		GlobalResources.getInstance().getAddRPC().addMarriage(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	public void setAddMarriagePopupEventHandler(AddMarriagePopupEventHandler handler){
