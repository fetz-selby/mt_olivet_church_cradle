...
public class SpecialOfferingApprovePopup extends CustomDraggablePopupPanel {

	private SpecialOfferingApprovePopupEventHandler handler;
	...
	interface OfferingApprovePopupUiBinder extends
			UiBinder<Widget, SpecialOfferingApprovePopup> {
	}
	
	public interface SpecialOfferingApprovePopupEventHandler{
		void onSpecialOfferingApproved(int id);
	}
	
	@UiField ReviewPopupTextArea descField;
	@UiField TextBox amountField, createdByField, dateField, memberField;
	...
	public SpecialOfferingApprovePopup(SpecialOfferingModel model) {
		this.model = model;
	...
	private void initComponent(){
		...
		dateField.setText(model.getCreatedDate());
		memberField.setText(Utils.getMember(model.getMemberId()));
	...
	private void showSuccess(){
		...
		titleErrorMessage.setInnerText("Special Offering Approved Successfully");
	}
	
	private void showRejectSuccess(){
		errorDiv.setClassName("alert alert-success");
		titleErrorMessage.setInnerText("Special Offering Rejected Successfully");
	}
	...
	private void showSuccessAndHide(){
		...
		Timer timer = new Timer() {
			...
			public void run() {
				hide();
				if(handler != null){
					handler.onSpecialOfferingApproved(model.getId());
				}
			}
		};
	...
	private void doUpdate(){
		if(isChecked){
			showLoading(true);
			GlobalResources.getInstance().getUpdateRPC().approveSpecialOffering(GlobalResources.getInstance().getModel().getId(), model, new AsyncCallback<Boolean>() {
	...
	private void doReject(){
		showLoading(true);
		GlobalResources.getInstance().getUpdateRPC().rejectSpecialOffering(GlobalResources.getInstance().getModel().getId(), model, new AsyncCallback<Boolean>() {
	...
	public void setSpecialOfferingApprovePopupEventHandler(SpecialOfferingApprovePopupEventHandler handler){
