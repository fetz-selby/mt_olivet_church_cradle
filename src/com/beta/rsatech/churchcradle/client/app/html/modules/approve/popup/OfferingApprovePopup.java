...
public class OfferingApprovePopup extends CustomDraggablePopupPanel {

	private OfferingApprovePopupEventHandler handler;
	...
	interface OfferingApprovePopupUiBinder extends
			UiBinder<Widget, OfferingApprovePopup> {
	}
	
	public interface OfferingApprovePopupEventHandler{
		void onOfferingApproved(int id);
	}
	
	@UiField ReviewPopupTextArea descField;
	@UiField TextBox amountField, createdByField, dateField;
	...
	public OfferingApprovePopup(OfferingModel model) {
		this.model = model;
	...
	private void showSuccess(){
		...
		titleErrorMessage.setInnerText("Offering Approved Successfully");
	}
	
	private void showRejectSuccess(){
		errorDiv.setClassName("alert alert-success");
		titleErrorMessage.setInnerText("Offering Rejected Successfully");
	}
	...
	private void showSuccessAndHide(){
		...
		Timer timer = new Timer() {
			...
			public void run() {
				hide();
				if(handler != null){
					handler.onOfferingApproved(model.getId());
				}
			}
		};
	...
	private void doUpdate(){
		if(isChecked){
			showLoading(true);
			GlobalResources.getInstance().getUpdateRPC().approveOffering(GlobalResources.getInstance().getModel().getId(), model, new AsyncCallback<Boolean>() {
	...
	private void doReject(){
		showLoading(true);
		GlobalResources.getInstance().getUpdateRPC().rejectOffering(GlobalResources.getInstance().getModel().getId(), model, new AsyncCallback<Boolean>() {
	...
	public void setOfferingApprovePopupEventHandler(OfferingApprovePopupEventHandler handler){
