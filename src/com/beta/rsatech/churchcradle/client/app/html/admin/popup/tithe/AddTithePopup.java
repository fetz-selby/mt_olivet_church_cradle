...
public class AddTithePopup extends Composite {

	private int stageIndex = 0;
	AddTithePopupEventHandler handler;
	private IsWizardable<TitheModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AddTithePopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AddTithePopup> {
	}
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.ADD_TITHE_TITLE);
		wizard = new StageOne();
	...
	private void initWizardableEvent(final IsWizardable<TitheModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<TitheModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, TitheModel model) {
	...
	private void showAddSuccess(){
		...
		titleErrorMessage.setInnerText("Tithe Added Successfully");
	}
	...
	private void doSaveOffering(TitheModel model){
		disableButton(true);
		GlobalResources.getInstance().getAddRPC().addTithe(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	public void setAddTithePopupEventHandler(AddTithePopupEventHandler handler){
