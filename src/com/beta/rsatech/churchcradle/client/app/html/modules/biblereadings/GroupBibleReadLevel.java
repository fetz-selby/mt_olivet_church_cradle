...
public class GroupBibleReadLevel extends Composite {

	private int yearFilter = new Date().getYear() < 1900?new Date().getYear()+1900:new Date().getYear();
	private GroupBibleReadLevelEventHandler handler;
	...
	interface GroupBibleReadLevelUiBinder extends
			UiBinder<Widget, GroupBibleReadLevel> {
	}
	
	public interface GroupBibleReadLevelEventHandler{
		void onMonthClicked(ArrayList<BibleReadingModel> modelList);
	}
	...
	private void doListing(String month, int year){
		GlobalResources.getInstance().getListRPC().getBibleReading(GlobalResources.getInstance().getModel().getChurchId(), month+","+year, new AsyncCallback<ArrayList<BibleReadingModel>>() {
	...
	public void setGroupBibleReadLevelEventHandler(GroupBibleReadLevelEventHandler handler){
