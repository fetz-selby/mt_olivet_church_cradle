...
public class GroupOnlineOfferingLevel extends Composite {

	private int yearFilter = new Date().getYear() < 1900?new Date().getYear()+1900:new Date().getYear();
	private GroupOnlineOfferingLevelEventHandler handler;
	...
	interface GroupBibleReadLevelUiBinder extends
			UiBinder<Widget, GroupOnlineOfferingLevel> {
	}
	
	public interface GroupOnlineOfferingLevelEventHandler{
		void onMonthClicked(ArrayList<OnlineOfferingPaymentModel> modelList);
	}
	...
	public GroupOnlineOfferingLevel() {
	...
	private void doListing(String month, int year){
		...
		GlobalResources.getInstance().getPaymentRPC().getOnlineOfferingPayments(GlobalResources.getInstance().getModel().getChurchId(), month+","+year, new AsyncCallback<ArrayList<OnlineOfferingPaymentModel>>() {
			
			@Override
			public void onSuccess(ArrayList<OnlineOfferingPaymentModel> result) {
				if(result != null){
					if(handler != null){
	...
	public void setGroupOnlineOfferingLevelEventHandler(GroupOnlineOfferingLevelEventHandler handler){
