...
public class AddOfferingPopup extends Composite {

	private int stageIndex = 0;
	AddOfferingPopupEventHandler handler;
	private IsWizardable<OfferingModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AddOfferingPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AddOfferingPopup> {
	}
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.ADD_OFFERING_TITLE);
		wizard = new StageOne();
	...
	private void initWizardableEvent(final IsWizardable<OfferingModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<OfferingModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, OfferingModel model) {
	...
	private void showAddSuccess(){
		...
		titleErrorMessage.setInnerText("Offering Added Successfully");
	}
	...
	private void doSaveOffering(OfferingModel model){
		disableButton(true);
		GlobalResources.getInstance().getAddRPC().addOffering(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	public void setAddOfferingPopupEventHandler(AddOfferingPopupEventHandler handler){
