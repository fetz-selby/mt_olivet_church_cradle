...
public class GroupOnlineDonationLevel extends Composite {

	private int yearFilter = new Date().getYear() < 1900?new Date().getYear()+1900:new Date().getYear();
	private GroupOnlineDonationLevelEventHandler handler;
	...
	interface GroupBibleReadLevelUiBinder extends
			UiBinder<Widget, GroupOnlineDonationLevel> {
	}
	
	public interface GroupOnlineDonationLevelEventHandler{
		void onMonthClicked(ArrayList<OnlineDonationPaymentModel> modelList);
	}
	...
	public GroupOnlineDonationLevel() {
	...
	private void doDateStyling(){
		yearWidget.addDateStyle(AppConstants.BG_PRIMARY);
	}
	...
	private void doListing(String month, int year){
		GlobalResources.getInstance().getPaymentRPC().getOnlineDonationPayments(GlobalResources.getInstance().getModel().getChurchId(), month+","+year, new AsyncCallback<ArrayList<OnlineDonationPaymentModel>>() {
			
			@Override
			public void onSuccess(ArrayList<OnlineDonationPaymentModel> result) {
				if(result != null){
					if(handler != null){
	...
	public void setGroupOnlineDonationLevelEventHandler(GroupOnlineDonationLevelEventHandler handler){
