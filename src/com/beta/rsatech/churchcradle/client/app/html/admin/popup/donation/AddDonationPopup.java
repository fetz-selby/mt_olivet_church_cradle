...
public class AddDonationPopup extends Composite {

	private int stageIndex = 0;
	AddDonationPopupEventHandler handler;
	private IsWizardable<DonationModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AddDonationPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AddDonationPopup> {
	}
	...
	private void initWizardableEvent(final IsWizardable<DonationModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<DonationModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, DonationModel model) {
	...
	private void doSaveOffering(DonationModel model){
		disableButton(true);
		GlobalResources.getInstance().getAddRPC().addDonation(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	public void setAddDonationPopupEventHandler(AddDonationPopupEventHandler handler){
