...
public class AddFuneralPopup extends Composite {

	private int stageIndex = 0;
	//private MemberModel model;
	AddFuneralPopupEventHandler handler;
	private IsWizardable<FAnnounceModel> wizard;
	private static AddMemberPopupUiBinder uiBinder = GWT
			.create(AddMemberPopupUiBinder.class);

	public interface AddFuneralPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, AddFuneralPopup> {
	}
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.ADD_FUNERAL_TITLE);
		wizard = new StageOne();
	...
	private void initWizardableEvent(final IsWizardable<FAnnounceModel> tmpWizard){
		tmpWizard.setHasWizardEvent(new HasWizardEvent<FAnnounceModel>() {

			@Override
			public void onValidateComplete(WizardStage stage, FAnnounceModel model) {
	...
	private void showAddSuccess(){
		...
		titleErrorMessage.setInnerText("Funeral Notice Added Successfully");
	}
	...
	private void doSaveFuneral(FAnnounceModel model){
		disableButton(true);
		GlobalResources.getInstance().getAddRPC().addFuneral(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	public void setAddFuneralPopupEventHandler(AddFuneralPopupEventHandler handler){
