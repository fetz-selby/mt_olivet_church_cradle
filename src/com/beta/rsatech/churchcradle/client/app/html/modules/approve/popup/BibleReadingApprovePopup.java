...
public class BibleReadingApprovePopup extends CustomDraggablePopupPanel {

	private BibleReadingApprovePopupEventHandler handler;
	...
	interface BibleReadingApprovePopupUiBinder extends
			UiBinder<Widget, BibleReadingApprovePopup> {
	}

	public interface BibleReadingApprovePopupEventHandler{
		void onBibleReadingApproved(int id);
	}
	
	@UiField ReviewPopupTextArea themeField;
	@UiField TextBox verseField, nameField, dateField, createdByField;
	...
	public BibleReadingApprovePopup(BibleReadingModel model) {
		this.model = model;
	...
	private void initComponent(){
		...
		themeField.setText(model.getTheme());
	...
	private void showSuccess(){
		...
		titleErrorMessage.setInnerText("Bible Reading Approved Successfully");
	}
	
	private void showRejectSuccess(){
		errorDiv.setClassName("alert alert-success");
		titleErrorMessage.setInnerText("Bible Reading Rejected Successfully");
	}
	...
	private void showSuccessAndHide(){
		...
		Timer timer = new Timer() {
			...
			public void run() {
				hide();
				if(handler != null){
					handler.onBibleReadingApproved(model.getId());
				}
			}
		};
	...
	private void doUpdate(){
		if(isChecked){
			showLoading(true);
			GlobalResources.getInstance().getUpdateRPC().approveBibleReading(GlobalResources.getInstance().getModel().getId(), model, new AsyncCallback<Boolean>() {
	...
	private void doReject(){
		showLoading(true);
		GlobalResources.getInstance().getUpdateRPC().rejectBibleReading(GlobalResources.getInstance().getModel().getId(), model, new AsyncCallback<Boolean>() {
	...
	public void setBibleReadingApprovePopupEventHandler(BibleReadingApprovePopupEventHandler handler){
