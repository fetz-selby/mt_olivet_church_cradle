...
public class OfferingApprovePage extends UIObject {

	private OfferingApprovePageEventHandler handler;
	...
	interface OfferingApprovePageUiBinder extends
			UiBinder<Element, OfferingApprovePage> {
	}

	public interface OfferingApprovePageEventHandler{
		void onOfferingEditInvoked(OfferingModel model);
	}
	
	
	@UiField UListElement ulListContainer;

	public OfferingApprovePage() {
		setElement(uiBinder.createAndBindUi(this));
	}

	private Element getLi(final OfferingModel model){
		...
		DOM.setEventListener(editIconAnchor, new EventListener() {
			...
			public void onBrowserEvent(Event event) {
				if(handler != null){
					handler.onOfferingEditInvoked(model);
				}
			}
		});
		...
		descDiv.setInnerText(Utils.getTruncatedText(model.getDescription(), AppConstants.APPROVE_OFFERING_DESCRIPTION_LIMIT));
	...
	private void doOfferingLoad(){
		GlobalResources.getInstance().getListRPC().getOfferingsListWithStatusFilter(GlobalResources.getInstance().getModel().getChurchId(), AppConstants.PENDING, new AsyncCallback<ArrayList<OfferingModel>>() {
			...
			public void onSuccess(ArrayList<OfferingModel> result) {
				...
				if(result != null){
					...
					for(OfferingModel model : result){
	...
	public void setOfferingApprovePageEventHandler(OfferingApprovePageEventHandler handler){
