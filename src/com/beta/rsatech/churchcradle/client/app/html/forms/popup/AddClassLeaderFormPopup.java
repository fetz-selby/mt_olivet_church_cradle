...
public class AddClassLeaderFormPopup extends CustomDraggablePopupPanel {
	...
	UiBinder<Widget, AddClassLeaderFormPopup> {
	}
	...
	public AddClassLeaderFormPopup() {
	...
	private void showSuccess(){
		...
		titleErrorMessage.setInnerText("Class Leader added successfully");
	}
	...
	private void addGroup(){
		...
		if(isLeaderExist(leader)){
			showError(true, "Leader already does exist.");
			return;
		}
		...
		GlobalResources.getInstance().getAddRPC().addClassLeader(model, GlobalResources.getInstance().getModel().getId(), new AsyncCallback<Integer>() {
	...
	private boolean isLeaderExist(String leaderName){

		HashMap<Integer, String> allleaders = GlobalResources.getInstance().getClassesLeadersMap();
		if(allleaders != null){
			for(Integer id : allleaders.keySet()){
				String tmpLeader = allleaders.get(id);
				if(tmpLeader.equalsIgnoreCase(leaderName)){
					return true;
				}
			}
		}
