...
public class OfferingPopup extends Composite {

	private int stageIndex = 0;
	private String month;
	AddOfferingPaymentPopupEventHandler handler;
	...
	public interface AddOfferingPaymentPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, OfferingPopup> {
	}
	...
	public OfferingPopup(String month) {
		this.month = month;
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.PAY_OFFERING_TITLE);
	...
	private void doSavePayment(PaymentModel model){
		...
		disableButton(true);
		GlobalResources.getInstance().getPaymentRPC().offeringPay(GlobalResources.getInstance().getModel().getChurchId(), GlobalResources.getInstance().getModel().getId(), model.getAmount(), model.getMonth(), Utils.getLocationPath("app.html"), new AsyncCallback<String>() {
	...
	public void setAddOfferingPaymentPopupEventHandler(AddOfferingPaymentPopupEventHandler handler){
