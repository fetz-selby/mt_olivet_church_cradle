...
public class TithePopup extends Composite {

	private int stageIndex = 0;
	private String month;
	AddTithePaymentPopupEventHandler handler;
	...
	public interface AddTithePaymentPopupEventHandler{
		void onClose();
	}

	interface AddMemberPopupUiBinder extends UiBinder<Widget, TithePopup> {
	}
	...
	public TithePopup(String month) {
		this.month = month;
	...
	private void initComponent(){
		titleSpan.setInnerText(AppConstants.ADD_PAYMENT_TITLE);
	...
	private void doSavePayment(PaymentModel model){
		...
		disableButton(true);
		GlobalResources.getInstance().getPaymentRPC().tithePay(GlobalResources.getInstance().getModel().getChurchId(), GlobalResources.getInstance().getModel().getId(), model.getAmount(), model.getMonth(), Utils.getLocationPath("app.html"), new AsyncCallback<String>() {
	...
	public void setAddTithePaymentPopupEventHandler(AddTithePaymentPopupEventHandler handler){
