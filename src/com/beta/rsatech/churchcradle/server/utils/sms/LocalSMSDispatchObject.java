...
public class LocalSMSDispatchObject {
	private static Connection con = DBConnection.getConnection();
	private String message, msisdn;
	private ChurchModel churchModel;

	public LocalSMSDispatchObject(String msisdn, String message, ChurchModel churchModel){
	...
	public void send(){
		if(isAppClean(churchModel, msisdn, message)){
			doDeduction(churchModel, msisdn, message);
		}else{
			System.out.println("App Has Expire or has Elapse SMS");
		}
	}
	
	private void doDeduction(ChurchModel model, String msisdn, String message){
		PreparedStatement prstmt = null;
		con = DBConnection.getConnection();

		try{
			prstmt = (PreparedStatement) con.prepareStatement("update churches set sms_left = ? where id = ? and status = 'A'");
			prstmt.setDouble(1, model.getSmsLeft() - (Utils.getSMSCount(msisdn) * Utils.getSMSLength(message)));
	...
	private boolean isAppClean(ChurchModel model, String msisdn, String message){
		double smsCount = Utils.getSMSCount(msisdn);
		if(smsCount > 0 && model.getSmsLeft() >= smsCount * Utils.getSMSLengthCount(message) && Utils.isAppValid(false, model.getExpireDate()) && Utils.getSMSLengthCount(message) > 0){
			return true;
		}
	...
	private void submitMessage() {
		...
		try {
			...
			dataStreamToServer.writeBytes("username="
					...
					+ URLEncoder.encode(churchModel != null ? churchModel.getDisplay() : "c. cradle", "UTF-8") + "&message="
			...
			saveToSentSMS(churchModel != null ? churchModel.getId() : 0, getSuccessfulCount(dataFromUrl));
